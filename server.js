...
function txToIncome(t, accountNameById) {
  return {
    plaidId: t.transaction_id,
    week: t.date,
    date: t.date,
    who: t.merchant_name || t.name || 'Income',
    amount: Math.round(Math.abs(Number(t.amount)) * 100) / 100,
    account: accountNameById[t.account_id] || 'Bank',
    note: 'Plaid income'
  };
}
...
app.post('/api/plaid/sync', async (_req, res) => {
  try {
    ...
    const forceFull = String(_req.query.full || '') === '1';
    console.log('[Plaid] Sync started' + (forceFull ? ' (full backfill)' : ''));
    let cursor = forceFull ? undefined : (state.cursor || undefined);
