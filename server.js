...
const appCategory = (t) => {
  ...
  if (primary === 'MEDICAL') return 'Medical';
  if (primary === 'GENERAL_MERCHANDISE') return 'Household Misc';
  if (primary === 'ENTERTAINMENT') return "Dallin's Fun Money";
...
app.post('/api/plaid/sync', async (_req, res) => {
  try {
    ...
    const accounts = ar.data.accounts.map(a => ({
      ...
      asOf: today,
      cat: a.type === 'depository' ? 'Cash & Savings' : (a.type === 'investment' ? 'Brokerage (Fidelity)' : 'Linked account'),
